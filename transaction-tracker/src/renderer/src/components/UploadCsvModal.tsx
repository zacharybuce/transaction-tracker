...
import { Bank, MonthName } from '@shared/types'
import Dropdown from './generalUi/Dropdown'
import { MONTHS } from '@shared/constants'

const YEARS = ['2023', '2024', '2025', '2026', '2027']
const BANKS: Bank[] = ['AIB', 'Revolut']
...
const UploadCsvModal = () => {
  const { showUploadCsvModal, setShowUploadCsvModal } = useTrackerProvider()
  ...
  const [bankDropdownValue, setBankDropdownValue] = useState<Bank>('Revolut')
  ...
  const handleBankDropdownSelect = useCallback((e: React.ChangeEvent<HTMLSelectElement>) => {
    setBankDropdownValue(e.target.value as Bank)
  }, [])

  const handleConfirm = useCallback(async () => {
    if (file && monthDropdownValue && yearDropdownValue && bankDropdownValue) {
      setLoading(true)
      //@ts-expect-error path exists on file type
      const path = file.path

      const res = await window.context.processCsv(
        path,
        monthDropdownValue,
        yearDropdownValue,
        bankDropdownValue
  ...
  }, [bankDropdownValue, file, handleClose, monthDropdownValue, yearDropdownValue])
      ...
      >
        <div className="relative p-4 w-full max-w-md max-h-full">
          <div className="relative bg-zinc-800 rounded-lg shadow">
            <ModalHeader />
            <div className="grid gap-4 grid-cols-3 px-6 py-2">
              <Dropdown
                label="Month"
              ...
              <Dropdown
                label="Bank"
                onChange={handleBankDropdownSelect}
                options={BANKS}
                currentValue={bankDropdownValue}
              />
            </div>
            {file ? (
